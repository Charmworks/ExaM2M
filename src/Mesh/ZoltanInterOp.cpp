...
class GeometricMeshElemAdapter : public Zoltan2::MeshAdapter< ZoltanTypes > {
  ...
  public:
    ...
    GeometricMeshElemAdapter(
      std::size_t nelem,
      const std::array< std::vector< real >, 3 >& centroid,
  ...
  private:
    ...
    const std::array< std::vector< real >, 3 >& m_centroid;
    //! Global mesh element ids
    const std::vector< long long >& m_elemid;
};
...
// *****************************************************************************
{
  ...
  using ZoltanTypes = Zoltan2::BasicUserTypes< real >;
